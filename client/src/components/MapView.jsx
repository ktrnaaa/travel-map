...
const MapView = () => {
  ...
  const markerInstance = L.marker([marker.lat, marker.lng], {icon: markerIcon})
	...
	if (formData.files.length > 0) {
		...
		console.log('Вибрані дані для відправки:', {title: formData.title, category: formData.category, lat: selectedMarker.lat, lng: selectedMarker.lng, private: formData.Private || false, file: formData.files[0]})
	...
	return (
		<div>
			...
			>
				<button onClick={() => setMapType('standard')}
				...
				<button onClick={() => setMapType('topographic')}
				        className="px-4 py-2 bg-blue-500 text-white rounded transition-all duration-300 hover:bg-blue-600">
					Топографічна
				</button>
			...
			{modalOpen && (
				<div
					className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-5 rounded shadow-lg z-1000">
						...
						<label className="block mb-2">
							<div className="relative">
								...
								<div className="flex justify-center">
									...
									>
										{formData.files.length > 0 ? `Вибрано: ${formData.files[0].name}` : 'Завантажити файл'}
									</label>
								</div>
							</div>
						</label>
